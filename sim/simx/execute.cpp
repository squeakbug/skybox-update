...
instr_trace_t* Emulator::execute(const Instr &instr, uint32_t wid) {
  ...
  visit_var(op_type,
    ...
    ,[&](TexType tex_type) {
      auto texArgs = std::get<IntrTexArgs>(instrArgs);
      switch (tex_type) {
      case TexType::TEX: {
        ...
        for (uint32_t t = thread_start; t < num_threads; ++t) {
          ...
          auto u     = rs1_data[t].i;
          auto v     = rs2_data[t].i;
          auto lod   = rs3_data[t].i;
          auto stage = texArgs.stage;
    ...
    ,[&](OmType om_type) {
      ...
      case OmType::OM: {
        ...
        for (uint32_t t = thread_start; t < num_threads; ++t) {
          if (!warp.tmask.test(t))
            continue;
          auto pos_face = rs1_data[t].i;
          auto color    = rs2_data[t].i;
          auto depth    = rs3_data[t].i;
